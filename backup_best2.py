...
class DeserializeClass:
    ...
    @classmethod
    def get_nested_structures(self, root_key, file_obj, model_obj, list_dict={}, counter=0):
        """
        Recurse list items
        """
        while True:
            list_key = root_key + '/' + str(counter)
            if list_key in file_obj:
                print(list_key)
                for key in file_obj.keys():
                    if file_obj.get(key).__class__.__name__ == 'Group':
                        list_dict[key] = dict()
                        self.get_nested_structures(key, file_obj[key], list_dict[key], counter)
                    else:
                        list_dict[key] = dict()
                        try:
                            key_value = file_obj.get(key).value
                            ...
                            if type(key_value).__name__ in ['ndarray']:
                                ...
                                list_dict[key] = key_value
                            continue
                print(root_key, list_dict)
                model_obj[root_key] = list()
                model_obj[root_key].append(list_dict)
            else:
                break
            counter += 1
        return list_dict
            ...
            for key in h5file_obj.keys():
                if h5file_obj.get(key).__class__.__name__ == 'Group':
                    ...
                    if list_key in h5file_obj:
                        #model_obj[key] = list()
                        item = self.get_nested_structures(key, h5file_obj, model_obj)
                        #model_obj[key].append(item)
                    else:
                        model_obj[key] = dict()
